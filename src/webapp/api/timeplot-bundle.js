...
Timeplot.createPlotInfo=function(A){return{id:("id" in A)?A.id:"p"+Math.round(Math.random()*1000000),dataSource:("dataSource" in A)?A.dataSource:null,eventSource:("eventSource" in A)?A.eventSource:null,timeGeometry:("timeGeometry" in A)?A.timeGeometry:new Timeplot.DefaultTimeGeometry(),valueGeometry:("valueGeometry" in A)?A.valueGeometry:new Timeplot.DefaultValueGeometry(),timeZone:("timeZone" in A)?A.timeZone:0,fillColor:("fillColor" in A)?((A.fillColor=="string")?new Timeplot.Color(A.fillColor):A.fillColor):null,fillGradient:("fillGradient" in A)?A.fillGradient:true,fillFrom:("fillFrom" in A)?A.fillFrom:Number.NEGATIVE_INFINITY,lineColor:("lineColor" in A)?((A.lineColor=="string")?new Timeplot.Color(A.lineColor):A.lineColor):new Timeplot.Color("#606060"),lineWidth:("lineWidth" in A)?A.lineWidth:1,dotRadius:("dotRadius" in A)?A.dotRadius:2,dotColor:("dotColor" in A)?A.dotColor:null,eventLineWidth:("eventLineWidth" in A)?A.eventLineWidth:1,eventOnClick:("eventOnClick" in A)?A.eventOnClick:null,eventOnMouseOver:("eventOnMouseOver" in A)?A.eventOnMouseOver:null,eventOnMouseOut:("eventOnMouseOut" in A)?A.eventOnMouseOut:null,showValues:("showValues" in A)?A.showValues:false,roundValues:("roundValues" in A)?A.roundValues:true,valuesOpacity:("valuesOpacity" in A)?A.valuesOpacity:75,bubbleWidth:("bubbleWidth" in A)?A.bubbleWidth:300,bubbleHeight:("bubbleHeight" in A)?A.bubbleHeight:200};
};
...
C.title="SIMILE Timeplot - http://www.simile-widgets.organ/timeplot/";
SimileAjax.DOM.registerEvent(C,"click",function(){window.location="http://www.simile-widgets.organ/timeplot/";
});
G.appendChild(C);
var E=this;
...
},paint:function(){var P=this._canvas.getContext("2d");
P.lineWidth=this._plotInfo.lineWidth;
P.lineJoin="miter";
if(this._dataSource){if(this._plotInfo.fillColor){if(this._plotInfo.fillGradient){var N=P.createLinearGradient(0,this._canvas.height,0,0);
N.addColorStop(0,this._plotInfo.fillColor.toString());
N.addColorStop(0.5,this._plotInfo.fillColor.toString());
N.addColorStop(1,"rgba(255,255,255,0)");
P.fillStyle=N;
}else{P.fillStyle=this._plotInfo.fillColor.toString();
}P.beginPath();
P.moveTo(0,0);
this._plot(function(Q,R){P.lineTo(Q,R);
});
if(this._plotInfo.fillFrom==Number.NEGATIVE_INFINITY){P.lineTo(this._canvas.width,0);
}else{if(this._plotInfo.fillFrom==Number.POSITIVE_INFINITY){P.lineTo(this._canvas.width,this._canvas.height);
P.lineTo(0,this._canvas.height);
}else{P.lineTo(this._canvas.width,this._valueGeometry.toScreen(this._plotInfo.fillFrom));
P.lineTo(0,this._valueGeometry.toScreen(this._plotInfo.fillFrom));
}}P.fill();
}if(this._plotInfo.lineColor){P.strokeStyle=this._plotInfo.lineColor.toString();
P.beginPath();
var H=true;
this._plot(function(Q,R){if(H){H=false;
P.moveTo(Q,R);
}P.lineTo(Q,R);
});
P.stroke();
}if(this._plotInfo.dotColor){P.fillStyle=this._plotInfo.dotColor.toString();
var A=this._plotInfo.dotRadius;
this._plot(function(Q,R){P.beginPath();
P.arc(Q,R,A,0,2*Math.PI,true);
P.fill();
});
}}if(this._eventSource){var N=P.createLinearGradient(0,0,0,this._canvas.height);
N.addColorStop(1,"rgba(255,255,255,0)");
P.strokeStyle=N;
P.fillStyle=N;
P.lineWidth=this._plotInfo.eventLineWidth;
P.lineJoin="miter";
var G=this._eventSource.getAllEventIterator();
while(G.hasNext()){var C=G.next();
var F=C.getColor();
F=(F)?new Timeplot.Color(F):this._plotInfo.lineColor;
var K=C.getStart().getTime();
var I=C.getEnd().getTime();
if(K==I){var L=F.toString();
N.addColorStop(0,L);
var D=this._timeGeometry.toScreen(K);
D=Math.floor(D)+0.5;
var E=D;
P.beginPath();
P.moveTo(D,0);
P.lineTo(D,this._canvas.height);
P.stroke();
var M=D-4;
var O=7;
}else{var L=F.toString(0.5);
N.addColorStop(0,L);
var D=this._timeGeometry.toScreen(K);
D=Math.floor(D)+0.5;
var E=this._timeGeometry.toScreen(I);
E=Math.floor(E)+0.5;
P.fillRect(D,0,E-D,this._canvas.height);
var M=D;
var O=E-D-1;
}var B=this._timeplot.putDiv(C.getID(),"timeplot-event-box",{left:Math.round(M),width:Math.round(O),top:0,height:this._canvas.height-1});
var J=this;
if(!B.instrumented){SimileAjax.DOM.registerEvent(B,"click",this._makeClickHandler(C));
SimileAjax.DOM.registerEvent(B,"mouseover",this._makeMouseOverHandler(C));
SimileAjax.DOM.registerEvent(B,"mouseout",this._makeMouseOutHandler(C));
B.instrumented=true;
}}}},_makeClickHandler:function(A){var B=this;
return function(E,C,G){if(B._plotInfo.eventOnClick){return B._plotInfo.eventOnClick(E,C,G,A,B);
}B._closeBubble();
var F=SimileAjax.DOM.getEventPageCoordinates(C);
var D=SimileAjax.DOM.getPageCoordinates(E);
B._bubble=SimileAjax.Graphics.createBubbleForPoint(F.x,D.top+B._canvas.height,B._plotInfo.bubbleWidth,B._plotInfo.bubbleHeight,"bottom");
A.fillInfoBubble(B._bubble.content,B._theme,B._timeGeometry.getLabeler());
};
},_makeMouseOverHandler:function(A){var B=this;
return function(D,C,E){if(B._plotInfo.eventOnMouseOver){return B._plotInfo.eventOnMouseOver(D,C,E,A,B);
}D.oldClass=D.className;
D.className=D.className+" timeplot-event-box-highlight";
};
},_makeMouseOutHandler:function(A){var B=this;
return function(D,C,E){if(B._plotInfo.eventOnMouseOut){return B._plotInfo.eventOnMouseOut(D,C,E,A,B);
}D.className=D.oldClass;
D.oldClass=null;
};
...
this._unit=("unit" in B)?B.unit:SimileAjax.NativeDateUnit;
this._linMap={direct:function(C){return C;
},inverse:function(C){return C;
}};
this._map=this._linMap;
if(!this._labeler){this._labeler=(this._unit&&("createLabeller" in this._unit))?this._unit.createLabeller(this._locale,this._timeZone):new Timeline.GregorianDateLabeller(this._locale,this._timeZone);
